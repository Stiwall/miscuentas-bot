...
// ─── IN-MEMORY ────────────────────────────────────────────────────────────
// pendingTx y userLang se persisten dentro del objeto de usuario en JSONBin
// para sobrevivir reinicios del servidor (Render free tier duerme frecuentemente).
// Solo se usan como cache rápido en memoria durante la sesión activa.
const pendingTx = {};
const userLang  = {};
...
// ─── DATA (con cache en memoria) ──────────────────────────────────────────
// Cache evita llamar a JSONBin en cada mensaje del bot.
// TTL 60s — desfase máximo entre bot y web es 1 minuto.
let _cache = null;
let _cacheTs = 0;
...
async function loadData() {
  ...
  try {
    ...
    if (record?.transactions && !record?.users) {
      _cache = { users: {} }; // migrate old format
    } else {
      _cache = record?.users ? record : { users: {} };
    }
    _cacheTs = Date.now();
    return _cache;
  } catch (e) {
    console.error('loadData:', e.message);
    if (_cache) return _cache; // devolver cache vencido antes que objeto vacío
    return { users: {} };
  }
}
...
async function saveData(data) {
  ...
  if (!r.ok) throw new Error(`HTTP ${r.status}`);
  // Actualizar cache tras guardar exitosamente
  _cache = data;
  _cacheTs = Date.now();
}

function getUser(data, id) {
  if (!data.users[id]) data.users[id] = { registered: false, transactions: [], budgets: {}, pending: null, lang: null };
  const u = data.users[id];
  if (!u.transactions) u.transactions = [];
  if (!u.budgets) u.budgets = {};
  if (!('registered' in u)) u.registered = true;
  // Clear stale PIN-related pending states from old versions
  if (u.pending && ['awaiting_login_pin','awaiting_new_pin','awaiting_pin_confirm','awaiting_change_pin_new','awaiting_change_pin_confirm'].includes(u.pending)) {
    u.pending = null;
    u.registered = true;
  }
  // Restaurar pendingTx y userLang en memoria desde JSONBin tras reinicio
  if (u.pendingTx && !pendingTx[id]) pendingTx[id] = u.pendingTx;
  if (u.lang && !userLang[id])       userLang[id]  = u.lang;
  return u;
}
...
async function handleText(msgText, chatId) {
  ...
  try {
    ...
    if (!user.registered) {
      ...
      user.lang = userLang[id]; // persistir en JSONBin
      await save();
    ...
    if (parsed?.cmd==='confirmar') {
      ...
      if (p) {
        user.transactions.push(p);
        delete pendingTx[id];
        user.pendingTx = null; // limpiar en JSONBin
        await save();
    ...
    if (parsed?.cmd==='cancelar') {
      if (pendingTx[id]) {
        ...
        user.pendingTx = null; // limpiar en JSONBin
        await save();
        return MSG.cancelled(getLang(id, msg));
      }
...
async function handlePhoto(msg, chatId) {
  ...
  try {
    ...
    // Persistir en JSONBin para sobrevivir reinicios
    const allData = await loadData();
    const u = getUser(allData, id);
    u.pendingTx = tx;
    allData.users[id] = u;
...
// ========== CORS (middleware global — un solo lugar) ==========
